...
//mocha needs to see describe globally. If you try putting it in a function, it wont excecute. (Unless my test wasn't good.)
    describe('CMAPI Map.View Unit Test Channels', function () {
        ...
        describe('Map.view.zoom Channel Set Tests', function () {
                    ...
                    meridian.sandbox.external.postMessageToParent = function (params) {
                        if (params.channel == 'map.status.ready') {
                            ...
                            map.events.register("zoomend", map, function(){
                                ...
                                // EXPECT: We expect that after the emit, our Zoom Level should be Zero.
                                expect(afterZoom_state).to.equal(0);
                                // EXPECT: Expect that the emit also sets our Longitude and Latitude to Zero.
                                expect(afterCenter_pos.lon).to.equal(0)
                                expect(afterCenter_pos.lat).to.equal(0);
                                done();
                            });
                            var beforeZoom_state = map.getZoom();
                            var beforeCenter_pos = map.getCenter();
                            // EXPECT: We expect our before Zoom + Coordinates to Match our manually entered data via map.setCenter.
                            expect(beforeZoom_state).to.equal(5);
                            expect(beforeCenter_pos.lon).to.be.above(38.86000).and.below(38.86100);
                            expect(beforeCenter_pos.lat).to.be.above(-77.059310).and.below(-77.059305);
        ...
        //
        //}); // describe
        //describe('Map.view.center Channel Set Tests', function () {
        //
