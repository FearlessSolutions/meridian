                    ...
                    meridian.sandbox.external.postMessageToParent = function (params) {
                        if (params.channel == 'map.status.ready') {
                            ...
                            meridian.sandbox.on('map.layer.create', function (params) {
                                afterLayerCreateCount = map.layers.length;
                                expect(afterLayerCreateCount).to.be.above(beforeLayerCreateCount); // confirmation that a layer was created
                                actualLayer = map.getLayersBy('layerId', 'cmapi')[0].layerId;
                                expect(actualLayer).to.equal('cmapi');  // actual layerId should equal the default layerId, 'cmapi'
                            });
                            meridian.sandbox.on('map.features.plot', function (params) {
                                confirmPlot = map.getLayersBy('layerId', 'cmapi')[0];  // grab layer that has cmapi layerId
                                expect(confirmPlot['featureId']).to.equal(payload.feature.features[0].featureId); // confirm feature by Id that it has been added to default layer and matches Id from payload
                                done();
                            });
                            meridian.sandbox.external.receiveMessage({
                                data: {
